...
//                        mBgView.getPaddingRight(), mBgView.getPaddingBottom());
//            }
//            mCenterTopView.setPadding(mCenterTopView.getPaddingLeft(), mLeftRightViewParent.getBottom() - mLeftTopView.getHeight(),
...
////    }
        ...
        if (mParent.getWidth() > 0 &&
                (mCenterTopView.getLeft() - mLeftTopView.getRight() < limit || mRightTopView.getLeft() - mCenterTopView.getRight() < limit))
        { //主菜单与左右菜单相交，无论是否与标题栏相交，主菜单高度都在左右菜单下方
            Log.e("cjztest", "主菜单与左右菜单相交");
            if (rectTitleBar.left < rectLeft.right || rectTitleBar.right > rectRight.left) {
                Log.e("cjztest", "主菜单与左右菜单相交，且与标题栏相交");
            } else {
        ...
        //样例代码：
        RelativeLayout.MarginLayoutParams marginLayoutParams = new RelativeLayout.MarginLayoutParams(mCenterTopView.getLayoutParams());
        marginLayoutParams.topMargin += 500;
        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mCenterTopView.getLayoutParams();
        ...
        mCenterTopView.setLayoutParams(params);
    }
}
